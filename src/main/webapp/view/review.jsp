...
<body>
	<div id="container">
			...
			<table>
				...
				<tr>
					...
					<td id="img">
						...
						<img src="/css/images/${bookName}Page.jpg">
						<br> <c:choose>
							<c:when
						...
						<div id="reviewDesc">
							...
							by ${authorName}(Allbooks Author)<br>
							${readerLogin}'s Review<br>
							<p>${reviewText }</p>
							...
							<c:choose>
								<c:when test="${sessionScope.logged == true}">
									<form:form action="/reader/dropLikeReview" method="GET" id="likeForm">
								...
								<c:when test="${sessionScope.logged == false}">
									<form:form action="/reader/loginPage" method="GET" id="likeForm">
										<input type="submit" value="Like"/>
									</form:form>
								</c:when>
							</c:choose>
		...
		<div id="communityReviews">
			...
			<c:choose>
			<c:when test="${sessionScope.logged == true }">
				<form:form action="submitComment" modelAttribute="comment" method="GET">
					<input type="hidden" name="reviewId" value="${reviewId}"/>
					<input type="hidden" name="readerLogin" value="${readerLogin}"/>
					<input type="hidden" name="bookId" value="${bookId}"/>
					...
					<input type="hidden" name="authorName" value="${authorName}"/>									
					Comment<br>
					<form:textarea id="reviewBox" path="text" required="required"/><br>
					<input type="hidden" name="bookName" value="magnusChase"/>
					<input type="submit" value="Submit">
				</form:form>
			</c:when>
			...
			<table>
				<c:forEach var="tempComment" items="${reviewComments}">
					...
					<tr>
						<td>
							<h4 id="h4">${tempComment.readerLogin}</h4>
							rated it ${tempComment.readerRating}
						</td>
					</tr>
					<tr>
						<td id="reviewTitleTable">
							${tempComment.text}
						</td>
					</tr>
					<tr>
